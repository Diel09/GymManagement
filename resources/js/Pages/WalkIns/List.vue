                    ...
                    <table v-if="list.length > 0" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr v-for="l in list" :key="l.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-4">{{ l.amount }}</td>
                                <td class="px-6 py-4">{{ formatDate(l.date) }}</td>
                                <td class="px-6 py-4">{{ formatTime(l.date) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
export default {
    ...
    methods: {
        async updateList() {
            ...
            try {
                const response = await axios.post('/fetch-list', { date: this.date });
